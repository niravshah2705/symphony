...
function buildDraftPrompt({ project, today, config }) {
  return [
    ...
    'Core Features, APIs & Integration, Quality & Hardening) with buildable issues.',
    '',
    ...
    '  • A relative T-shirt size (XS, S, M, L, or XL) reflecting effort/complexity.',
...
function buildExtractPrompt({ project, today, config, draft, research }) {
  const shape = [
    ...
    '        "tshirtSize": "XS"|"S"|"M"|"L"|"XL" (relative effort/complexity of this task),',
  ...
  return [
    ...
    'Give every issue a "tshirtSize" (XS/S/M/L/XL) reflecting its relative effort and complexity:',
    'XS = trivial/localized change; S = small; M = moderate; L = large/multi-component; XL = very large.',
...
function buildResumePrompt({ project, milestones, config, research }) {
  const list = milestones
    ...
    .join('\n');
  const shape =
    '{ "milestones": [ { "name": string, "evaluationCriteria": string, "issues": [ { "title": string, "description": string, "priority": 0-4, "tshirtSize": "XS"|"S"|"M"|"L"|"XL", "evaluationCriteria": string } ] } ] }';
  return [
    ...
    'build, implementation approach, edge cases) — not a one-liner. Give every issue a',
    '"tshirtSize" (XS/S/M/L/XL) reflecting its relative effort/complexity.',

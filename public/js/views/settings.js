...
const ALL_PROVIDERS = Object.freeze(['ollama', 'lmstudio', 'codex', 'claude']);
...
function buildLlmSection(ctx, rebuild) {
  ...
  return section('Task Models', summary, incomplete, [
    el('p', { class: 'muted', style: 'font-size:13px;margin-top:0' }, 'Assign a model to each task type. Each role picks any provider — local (Ollama / LM Studio) or hosted (OpenAI / Anthropic) — plus a model and model-supported reasoning level. Recommended context, output, and sampling values are applied automatically; advanced values remain customizable.'),
...
function presetSlot(ctx, role, rebuild) {
  ...
  const deploymentHint = deployment === 'local'
    ? ' Runs on this machine (Ollama / LM Studio).'

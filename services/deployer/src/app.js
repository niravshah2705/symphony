...
const { executeDeploymentStage } = require('@ai-fleet/shared/agent/pipeline-stage-runtime');
...
function createApp(options = {}) {
  ...
  const execute = inCommandWorkspace(options.execute || executeDeploymentStage, initStore);
  ...
  app.get('/healthz', (req, res) => res.json({ status: 'ok', stage: 'deploy' }));
  app.get('/readyz', (req, res) => res.json({ status: 'ready', stage: 'deploy' }));
  ...
  app.use((error, req, res, next) => { // eslint-disable-line no-unused-vars
    ...
    logger.error(`deployer stage request failed: ${safeError(error).message}`);
    return res.status(503).json({ error: 'Deployer stage completion could not be delivered.' });
  });
  return app;
}

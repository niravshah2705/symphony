...
function command() {
  const stages = ['plan', 'code', 'test', 'deploy'];
  const preflight = createPreflightSnapshot({
    runId: 'deployer-service-run', organizationId: 'org-1', projectId: 'project-1', requestedStages: stages,
    repository: {}, workItem: {}, stageConfiguration: { deploy: {} }, policy: {},
  }, { clock: () => NOW });
  return createStageCommandV1({
    runId: preflight.runId, organizationId: preflight.organizationId, projectId: preflight.projectId,
    requestedStages: stages, preflight, stage: 'deploy', attempt: 1, input: {},
  }, { clock: () => NOW });
}
...
test('deployer exposes canonical internal and Pub/Sub push-compatible stage routes', async () => {
  const executed = [];
  const published = [];
  const app = createApp({
    initStore: async () => {},
    execute: async (value) => { executed.push(value); return { output: { summary: 'deployed' } }; },
  ...
  assert.equal(published[0].stage, 'deploy');
});

test('deployer parser accepts the exact push-body budget and rejects one byte over', async () => {
  const app = createApp({
    internalAuth: (req, res, next) => next(),
    pushAuth: (req, res, next) => next(),
    logger: { error() {}, warn() {} },

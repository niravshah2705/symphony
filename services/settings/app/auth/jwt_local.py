...
The settings service is normally reached behind the gateway with Firebase ID
tokens, but it keeps the org service's local-JWT path so it can be exercised
directly (and so tests can mint a token without an IdP). Security notes
(authentication-failures.md / oauth-oidc.md):

...
- `InMemoryDb` — a dependency-free in-process fake used by the test suite so the
  cascade + authz/tenant-isolation matrix runs without the emulator.

Collection paths are strings like ``"organizations/{org}/settings"`` or
``f"users/{uid}/settings"``. Tenant isolation is structural: every org-owned
policy lives under ``organizations/{org_id}/...`` so a query cannot cross
tenants.
"""

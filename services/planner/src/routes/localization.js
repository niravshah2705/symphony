...
async function suggestionsForRequest(req, { geolocate = locateIp, geolocateCurrent = locateCurrentIp } = {}) {
  ...
  // The planner is internal, so its socket peer is the gateway. The gateway
  // injects the already trust-proxy-resolved browser address on this one route;
  // direct/test callers retain the local requestIp fallback.
  const forwardedIp = req && req.headers && req.headers['x-ai-fleet-client-ip'];
  const ip = typeof forwardedIp === 'string' && forwardedIp.trim()
    ? forwardedIp.trim()
    : requestIp(req);
  let location = null;
  if (ip && isPublicIp(ip)) {
...
// Translation is LLM-backed, so the complete surface lives with the planner.

...
test('planner localization router exposes the agreed suggestions and translation routes', () => {
...
test('suggestions use the gateway-resolved client address rather than the planner socket peer', async () => {
  let lookedUpIp = null;
  const payload = await suggestionsForRequest(
    {
      query: {},
      headers: { 'x-ai-fleet-client-ip': '8.8.4.4', 'accept-language': 'en-US' },
      ip: '10.0.0.2',
    ...
    },
    {
      geolocate: async (ip) => {
        lookedUpIp = ip;
        return { countryCode: 'IN', region: 'Maharashtra' };
      },
    },
  );
  assert.equal(lookedUpIp, '8.8.4.4');
  assert.equal(payload.recommendedLocale, 'mr-IN');
});

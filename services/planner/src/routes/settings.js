...
// are provider-flexible and reuse whichever provider block they name.
const ROLE_KEYS = Object.freeze({
  ...
  deployment: { provider: 'deploymentLlmProvider', preset: 'deploymentLlmPresetId' },
});
...
// Every purpose role now has a live pipeline consumer, so the persisted
// estimate must use the same four-role basis shown in the browser.
const COSTED_ROLES = MODEL_ROLES;
 ...
 * Append a no-secret selection record for later tuning: the applied tier, each
   * task role's provider/preset/model ids, an indicative monthly cost, and the
...
/** Providers a role may select — task roles accept both BYoM and hosted. */
...
function publicSettings() {
  ...
  return {
    ...
    // Task-model roles. Each names a provider and reuses that provider's block:
    // thinking → planner, execution → coder, testing → reserved tool calling,
    // deployment → deployer/release stage.
    ...
    deploymentLlmProvider: s.deploymentLlmProvider || s.llmProvider || 'ollama',
    deploymentLlmPresetId: s.deploymentLlmPresetId || 'custom',
...
// Planner-owned heavy settings surface, reverse-proxied by the gateway at the
// same public /api/settings URL.
// GET /api/settings
router.get('/', (req, res) => {
...
router.put('/llm-preset', (req, res) => {
  ...
  // Provider-class slots enforce the local/hosted split; task roles accept any
...
// PUT /api/settings/complexity — apply a curated complexity tier to every task
// role at once (the settings slider). Only the tier id is read from the
...
// 'execution'|'testing'|'deployment' }. Defaults to the global (hosted) slot.
// Task roles accept any provider; provider-class slots stay pinned.

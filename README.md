...
- **Local-first (Ollama or LM Studio):** run a tool-capable model locally and choose its preset. Point any task role (Thinking / Execution / Testing) at a local provider and nothing is sent to a hosted LLM for that role. Ollama receives `think`; compatible LM Studio GPT-OSS runtimes receive `reasoning_effort`.
...
in **Task Models** (each of Thinking / Execution / Testing picks any local Ollama/LM Studio or hosted Codex/Claude model).

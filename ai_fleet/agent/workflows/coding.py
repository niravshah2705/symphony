"""Coding workflow — the code-writer agent (port of workflows/coding.workflow.js).

A shell-backed deep agent (LocalShellBackend rooted at an isolated git workspace)
that loads the standard code skills and the injected linear_graphql tool, then
works a single Linear ticket end-to-end.
"""
...
try:
    from ai_fleet.agent.tools.index import TOOL_NAMES
except Exception:  # tools module not yet importable in some contexts
    TOOL_NAMES = []


def build_workflow_prompt(config) -> str:
    return "\n".join(
        [
            ...
            "   and confirm the server applied the review label " + f"`{config.prLabel}`.",
...
WORKFLOW = {
    ...
    "systemPrompt": lambda ctx=None: build_workflow_prompt(CONFIG.CODER),
    "buildWorkflowPrompt": build_workflow_prompt,
}
